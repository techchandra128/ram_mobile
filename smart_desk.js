...
function sdShowAddModal(type) {
    ...
    if (type === 'files') {
        ...
        title.textContent = 'Add Playlists to Smart Desk';
        const allPlaylists = JSON.parse(localStorage.getItem('ram_playlists') || '[]');
        if (allPlaylists.length === 0) {
            body.innerHTML = '<div class="sd-modal-empty">No playlists yet.</div>';
        } else {
...
function sdShowRemoveModal(type) {
    ...
    if (type === 'files') {
        ...
        title.textContent = 'Remove Playlists from Smart Desk';
        const allPlaylists = JSON.parse(localStorage.getItem('ram_playlists') || '[]');
        const myPlaylists = sdPlaylistShortcuts.map(pid => allPlaylists.find(p => p.id === pid)).filter(Boolean);
        if (myPlaylists.length === 0) {
            body.innerHTML = '<div class="sd-modal-empty">No playlists on Smart Desk yet.</div>';
        } else {
